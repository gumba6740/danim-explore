...
C = 0.15  # 취향 기준선. cos가 이보다 낮으면 페널티(점수 깎임)
...
def _main_score(post, now):
    ...
    """
    return _popularity(post) * _sigmoid_decay(_age_days(post, now))
...
def _sub_score(post, now):
    ...
    로그를 씌운 조회수를 점수에 반영.
    """
    return _popularity(post) * math.log1p(post.view_count) * _sigmoid_decay(_age_days(post, now))


# 코드워드 개인화 ============================================================
def _post_codewords(post):
    """post 에 연결된 PostEmbedding.codewords 를 반환. 없으면 None.
    (select_related('vector') 로 미리 당겨오면 추가 쿼리 없음)"""
    try:
        emb = post.vector            # OneToOne related_name='vector'
    except Exception:
        return None
    if emb is None:
        return None
    return emb.codewords or None


def _user_norm(taste_counts):
    """유저 카운트 벡터의 L2 노름. 코사인 분모용으로 1회 계산."""
    if not taste_counts:
        return 0.0
    return math.sqrt(sum(float(v) * float(v) for v in taste_counts.values()))


def _post_affinity(post, taste_counts, u_norm):
    """
...
def _affinity_boosted(base_score_fn, alpha, taste_counts):
    """기본 스코어 함수에 코드워드 개인화 점수를 가미하는 고차 함수.
    affinity = exp(BETA * cos) 형태는 기존 덴스 벡터 방식과 동일."""
    u_norm = _user_norm(taste_counts)

    def score_fn(post):
        base = base_score_fn(post)
        if u_norm == 0.0:
            return base
        cos = _post_affinity(post, taste_counts, u_norm)
        # if cos <= 0.0:
        #     return base
        affinity = math.exp(PERSONALIZATION_BETA * (cos - C))
        # alpha 는 0 이상 1 이하
...
def new_order(now, seed, *, taste_counts=None, alpha=0.0):
    """
    ...
    personalize = bool(taste_counts) and alpha > 0
    ...
    # rookie 점수에 개인화 반영
    if personalize:
        scored_fn = _affinity_boosted(_rookie_score, alpha * NEW_SCORED_PERSONAL, taste_counts)
    else:
...
def main_order(now, seed, *, taste_counts=None, alpha=0.0):
    ...
    score_fn = _affinity_boosted(base_score_fn, alpha, taste_counts) if personalize else base_score_fn
    return _weighted_order(pool, score_fn, seed=f"{seed}:main")[:MAIN_CUT]


def sub_order(now, seed, *, taste_counts=None, alpha=0.0):
...
def demo_order(now, seed, *, taste_counts=None, alpha=0.0, limit=300):
    posts = list(
        Post.objects.select_related("user", "vector")
        .annotate(reaction=F("like_count") + F("comment_count"))
    )
    u_norm = _user_norm(taste_counts)
    if u_norm == 0.0:
    ...
    def score(p):
        cos = _post_affinity(p, taste_counts, u_norm)
        affinity = math.exp(PERSONALIZATION_BETA * (cos - C))   # cos<C 깎임, cos>C 부스트
        pop = math.log1p(p.like_count + p.comment_count)
        return pop * (affinity ** alpha)
